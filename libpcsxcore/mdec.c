...
__inline void putlinebw15(u16 *image, int *Yblk) {
	int i;
...
__inline void putquadrgb15(u16 *image, int *Yblk, int Cr, int Cb) {
	int Y, R, G, B;
	int A = (mdec.reg0 & MDEC0_STP) ? 0x8000 : 0;
	R = MULR(Cr);
	G = MULG2(Cb, Cr);
...
__inline void yuv2rgb15(int *blk, unsigned short *image) {
	int x, y;
...
__inline void putlinebw24(u8 * image, int *Yblk) {
	int i;
	unsigned char Y;
...
__inline void putquadrgb24(u8 * image, int *Yblk, int Cr, int Cb) {
	int Y, R, G, B;
...
//else
	mdec.rl = (u16 *)&psxM_2[0x100000];//teste
}
...
//	DMA_INTERRUPT(1);}
//else{
	HW_DMA1_CHCR_2 &= SWAP32(~0x01000000);
	DMA_INTERRUPT_2(1);//}//teste
	return;
}
*/
