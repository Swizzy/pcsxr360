...
//if (use_vm){
//	ptr = (void *)PSXM(tmpHead.t_addr);}
//else{
	ptr = (void *)PSXM_2(tmpHead.t_addr);//}//teste
...
//else{
	memcpy((void *)PSXM_2(addr), buf + 12, 2048);//}//teste
...
//else
		fread(psxM_2 + 0x10000, 0x61000, 1, f);//teste
		fclose(f);
	}
}
...
//else{
	fread(PSXM_2(SWAP32(tmpHead.t_addr)), SWAP32(tmpHead.t_size), 1, tmpFile);//}//teste
				fclose(tmpFile);
				psxRegs.pc = SWAP32(tmpHead.pc0);
				psxRegs.GPR.n.gp = SWAP32(tmpHead.gp0);
				psxRegs.GPR.n.sp = SWAP32(tmpHead.s_addr); 
...
//else{
	fread(PSXM_2(section_address), section_size, 1, tmpFile);//}//teste
				break;
...
//else{
	fread(PSXM_2(SWAP32(section.s_paddr)), SWAP32(section.s_size), 1, tmpFile);//}//teste
					} else {
//if(use_vm){
//	memset(PSXM(SWAP32(section.s_paddr)), 0, SWAP32(section.s_size));}
//else{
	memset(PSXM_2(SWAP32(section.s_paddr)), 0, SWAP32(section.s_size));//}//teste
					}
				}
				break;
...
//else{
	gzwrite(f, psxM_2, 0x00200000);
	gzwrite(f, psxR_2, 0x00080000);
	gzwrite(f, psxH_2, 0x00010000);//}//teste
...
//else{
	gzread(f, psxM_2, 0x00200000); 
	gzread(f, psxR_2, 0x00080000); 
