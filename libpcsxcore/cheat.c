...
void ApplyCheats() {
	...
	for (i = 0; i < NumCheats; i++) {
		...
		for (j = Cheats[i].First; j < endindex; j++) {
			...
			switch (type) {
				case CHEAT_CONST8:
					psxMu8ref_2(addr) = (u8)val;
					break;

				case CHEAT_CONST16:
					psxMu16ref_2(addr) = SWAPu16(val);
					break;

				case CHEAT_INC16:
					psxMu16ref_2(addr) = SWAPu16(psxMu16_2(addr) + val);
					break;
				...
				case CHEAT_SLIDE:
					...
					if (type == CHEAT_CONST8) {
						...
						for (k = 0; k < ((addr >> 8) & 0xFF); k++) {
							psxMu16ref_2(taddr) = SWAPu16(val);
							taddr += (s8)(addr & 0xFF);
							val += (s8)(CheatCodes[j - 1].Val & 0xFF);
						}
					}
	...
	//					psxMu8ref(taddr + k) = PSXMu8(addr + k);
						psxMu8ref_2(taddr + k) = PSXMu8_2(addr + k);//teste
					}
					break;

				case CHEAT_EQU8:
//					if (PSXMu8(addr) != (u8)val)
                    if (PSXMu8_2(addr) != (u8)val)//teste
...
//			if (PSXMu8(i) == val) {
	...
	//		if (PSXMu8(SearchResults[i]) == val) {
			if (PSXMu8_2(SearchResults[i]) == val) {//teste
...
//			if (PSXMu16(SearchResults[i]) == val) {
		    if (PSXMu16_2(SearchResults[i]) == val) {//teste
				SearchResults[j++] = SearchResults[i];
			}
		}
...
//			if (PSXMu32(i) == val) {
	...
	//		if (PSXMu32(SearchResults[i]) == val) {
			if (PSXMu32_2(SearchResults[i]) == val) {//teste
				SearchResults[j++] = SearchResults[i];
			}
		}
...
//			if (PSXMu8(i) >= min && PSXMu8(i) <= max) {
			if (PSXMu8_2(i) >= min && PSXMu8_2(i) <= max) {//teste
				CheatSearchAddResult(i);
			}
		}
	}
...
//			if (PSXMu8(SearchResults[i]) >= min && PSXMu8(SearchResults[i]) <= max) {
			if (PSXMu8_2(SearchResults[i]) >= min && PSXMu8_2(SearchResults[i]) <= max) {//teste
				SearchResults[j++] = SearchResults[i];
			}
		}
...
//			if (PSXMu16(i) >= min && PSXMu16(i) <= max) {
			if (PSXMu16_2(i) >= min && PSXMu16_2(i) <= max) {//teste
				CheatSearchAddResult(i);
			}
		}
	}
...
//			if (PSXMu16(SearchResults[i]) >= min && PSXMu16(SearchResults[i]) <= max) {
            if (PSXMu16_2(SearchResults[i]) >= min && PSXMu16_2(SearchResults[i]) <= max) {//teste
				SearchResults[j++] = SearchResults[i];
			}
		}
...
//			if (PSXMu32(i) >= min && PSXMu32(i) <= max) {
			if (PSXMu32_2(i) >= min && PSXMu32_2(i) <= max) {//teste
				CheatSearchAddResult(i);
			}
		}
	}
...
//			if (PSXMu32(SearchResults[i]) >= min && PSXMu32(SearchResults[i]) <= max) {
			if (PSXMu32_2(SearchResults[i]) >= min && PSXMu32_2(SearchResults[i]) <= max) {//teste
				SearchResults[j++] = SearchResults[i];
			}
		}
...
//		if (PSXMu8(SearchResults[i]) - PrevMu8(SearchResults[i]) == val) {
		if (PSXMu8_2(SearchResults[i]) - PrevMu8(SearchResults[i]) == val) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PSXMu16(SearchResults[i]) - PrevMu16(SearchResults[i]) == val) {
		if (PSXMu16_2(SearchResults[i]) - PrevMu16(SearchResults[i]) == val) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PSXMu32(SearchResults[i]) - PrevMu32(SearchResults[i]) == val) {
		if (PSXMu32_2(SearchResults[i]) - PrevMu32(SearchResults[i]) == val) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PrevMu8(SearchResults[i]) - PSXMu8(SearchResults[i]) == val) {
		if (PrevMu8(SearchResults[i]) - PSXMu8_2(SearchResults[i]) == val) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PrevMu16(SearchResults[i]) - PSXMu16(SearchResults[i]) == val) {
		if (PrevMu16(SearchResults[i]) - PSXMu16_2(SearchResults[i]) == val) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PrevMu32(SearchResults[i]) - PSXMu32(SearchResults[i]) == val) {
		if (PrevMu32(SearchResults[i]) - PSXMu32_2(SearchResults[i]) == val) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PrevMu8(SearchResults[i]) < PSXMu8(SearchResults[i])) {
		if (PrevMu8(SearchResults[i]) < PSXMu8_2(SearchResults[i])) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PrevMu16(SearchResults[i]) < PSXMu16(SearchResults[i])) {
		if (PrevMu16(SearchResults[i]) < PSXMu16_2(SearchResults[i])) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
...
//		if (PrevMu32(SearchResults[i]) < PSXMu32(SearchResults[i])) {
		if (PrevMu32(SearchResults[i]) < PSXMu32_2(SearchResults[i])) {//teste
			SearchResults[j++] = SearchResults[i];
		}
	}
